...
export default class Vector extends EventTarget {
  ...
  constructor(name, location, grid, options) {
    ...
    if (typeof location == "function") {
      ...
      if (initialLocation.p1 === undefined) {
        initialLocation.p1 = {x: 0, y: 0}
      }
    }
    this.#makeLine(initialLocation.p1, initialLocation.p2, options.styles);

    if (options.resizable) {
      // TODO This (and its usages) assume dragging at the tip (p2) only
      this.dragPointSvg = makeResizable(
        this.line, initialLocation.p2, {x: 'x2', y: 'y2'},
        options.allowedIncrementSize || 10
      );
    }

    if (options.label) {
      this.#makeLabel(initialLocation.p1, initialLocation.p2, options);
    }
